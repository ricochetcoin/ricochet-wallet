...
const char DEFAULT_WALLET_FILE_NAME[] = "ricochet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "199.247.5.216:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("ricochetwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("ricochetwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("ricochetwallet.desktop");
  ...
  res = autorunSettings.contains("RicochetWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("RicochetWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.ricochet.ricochetwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Ricochet Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("RicochetWallet", appPath);
    } else {
      autorunSettings.remove("RicochetWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\ricochet", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:ricochet");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\ricochet\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\ricochet\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("ricochetwallet.cfg"));
