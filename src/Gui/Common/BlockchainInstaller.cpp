...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_ricochetDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfRicochetBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 ricochetHeight;
  if (!checkIfBlockchainOutdated(currentHeight, ricochetHeight)) {
    return;
  }
  ...
  QuestionDialog dlg(tr("Blockchain installation"), QString(questionStringTemplate).arg(ricochetHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::checkIfRicochetBlockchainExists() const {
  return m_ricochetDir.exists() && m_ricochetDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_ricochetDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::ricochetBlockchainInfo() const {
  return QFileInfo(m_ricochetDir.absoluteFilePath(m_blocksFileName));
}
...
void BlockchainInstaller::installBlockchain() {
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_ricochetDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_ricochetDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
